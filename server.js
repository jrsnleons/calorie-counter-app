...
app.post("/api/auth/google", async (req, res) => {
    ...
    try {
        ...
        if (!user) {
            ...
            await db.run(`UPDATE users SET name = ?, avatar = ? WHERE id = ?`, [
                googleName,
                googlePicture,
                user.id,
            ]);
...
app.post("/api/analyze", async (req, res) => {
    ...
    try {
        ...
        // NOTE: We save data.short_title into 'food_name'
        await db.run(
            `INSERT INTO meals (user_id, food_name, calories, protein, carbs, fat, items, date, meal_type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [
                req.session.userId,
                data.short_title || "Scanned Meal",
                data.total_calories,
...
app.post("/api/weight", async (req, res) => {
    ...
    try {
        ...
        await db.run("UPDATE users SET weight = ? WHERE id = ?", [
            weight,
            req.session.userId,
        ]);
...
app.post("/api/user/onboarding", async (req, res) => {
    ...
    try {
        await db.run(
            `UPDATE users SET height = ?, weight = ?, age = ?, gender = ?, activity_level = ?, goal = ?, tdee = ? WHERE id = ?`,
            [
                height,
                weight,
                age,
...
app.post("/api/history", async (req, res) => {
    if (!req.session.userId)
        return res.status(401).json({ error: "Unauthorized" });
    const { food_name, calories, protein, carbs, fat, items, meal_type } =
        req.body; // Added meal_type
    const date = new Date().toISOString().split("T")[0];

    try {
        await db.run(
            `INSERT INTO meals (user_id, food_name, calories, protein, carbs, fat, items, meal_type, date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [
                req.session.userId,
                food_name,
                calories,
